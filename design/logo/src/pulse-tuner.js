...
function hubMarkup(variant, v) {
  if (variant === 'reuleaux') {
    ...
    return `<path d="M${f(V[0][0])} ${f(V[0][1])} ${a(V[1])} ${a(V[2])} ${a(V[0])} Z" class="hub"/>`;
  }
  if (variant === 'hexagon') {
    // pointy-top hexagon; the three arms exit at alternating vertices.
    const V = [-90, -30, 30, 90, 150, 210].map((d) => vtx(v, d));
    return `<path d="M${V.map((p) => `${f(p[0])} ${f(p[1])}`).join(' L')} Z" class="hub"/>`;
  }
  // diamond (default) — a square on its corner, slightly tilted.
  const side = f(v * Math.SQRT2);
  const xy = f(32 - (v * Math.SQRT2) / 2);
  return `<rect x="${xy}" y="${xy}" width="${side}" height="${side}" rx="1.5" transform="rotate(50 32 32)" class="hub"/>`;
}

function buildMark({ variant, strokeW, nodeR, hub }) {
  let out = '';
  for (const d of ARMS) {
    out += `<path d="${d}" class="route" stroke-width="${strokeW}" stroke-linecap="round"/>`;
  }
  out += hubMarkup(variant, hub);
  for (const [cx, cy] of NODES) {
    out += `<circle cx="${cx}" cy="${cy}" r="${nodeR}" class="node"/>`;
  }
  return out;
}

const DEFAULTS = { strokeW: 5, nodeR: 5.5, hub: 9 };
