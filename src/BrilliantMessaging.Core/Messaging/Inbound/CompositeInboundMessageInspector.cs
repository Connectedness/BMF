...
public sealed class CompositeInboundMessageInspector : IInboundMessageInspector
{
    ...
    /// <exception cref="BrilliantMessaging.GuardClauses.Exceptions.EmptyCollectionException">Thrown when <paramref name="inspectors" /> is default or empty.</exception>
    /// <exception cref="BrilliantMessaging.GuardClauses.Exceptions.ExistingItemException">Thrown when an inspector entry is <see langword="null" />.</exception>
    public CompositeInboundMessageInspector(ImmutableArray<IInboundMessageInspector> inspectors)
    {
        _inspectors = inspectors
           .MustNotBeDefaultOrEmpty()
           .MustNotContainNull(nameof(inspectors));
    }
