# BMF RabbitMQ Transport

## Automatic Recovery

BMF requires RabbitMQ.Client automatic connection recovery. Keep
`ConnectionFactory.AutomaticRecoveryEnabled` set to `true`. RabbitMQ.Client owns
reconnection attempts and BMF continues to use the same autorecovering
...
RabbitMQ cluster nodes should use a consistent `channel_max`. BMF validates its
