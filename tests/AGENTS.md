...
- `dotnet test --solution BMF.slnx --no-build` currently forwards `--report-trx`, which the xUnit v3 Microsoft Testing Platform runner rejects. Running test projects directly with `dotnet run --project <test-project> --no-build` works.
